...
async function main() {
    ...
    try {
        ...
        if (!Array.isArray(tables) || tables.length === 0) {
            throw new Error("Table 'users' does not exist. Create it first (users table with role/status).");
        }
        const password_hash = bcryptjs_1.default.hashSync(ADMIN_PASSWORD, 10);
        // Check if exists
        const [existingRows] = await conn.execute("SELECT id FROM users WHERE email = ? LIMIT 1", [ADMIN_EMAIL]);
        if (Array.isArray(existingRows) && existingRows.length > 0) {
         ...
         SET full_name = ?,
             ...
             denied_at = NULL,
             denied_reason = NULL
         WHERE id = ?`, [ADMIN_NAME, password_hash, id]);
            console.log(`✅ Updated admin: ${ADMIN_EMAIL} (id=${id})`);
        }
        else {
            const [res] = await conn.execute(`INSERT INTO users (email, password_hash, full_name, role, status, approved_at)
         VALUES (?, ?, ?, 'admin', 'approved', NOW())`, [ADMIN_EMAIL, password_hash, ADMIN_NAME]);
            const id = res?.insertId;
            console.log(`✅ Created admin: ${ADMIN_EMAIL} (id=${id})`);
        }
        const [finalRows] = await conn.execute(`SELECT id, email, full_name, role, status, created_at, approved_at
       FROM users WHERE email = ? LIMIT 1`, [ADMIN_EMAIL]);
