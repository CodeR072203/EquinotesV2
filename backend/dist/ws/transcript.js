...
exports.parseChannelFromReq = parseChannelFromReq;
exports.isServerReady = isServerReady;
exports.safeParseControlMessage = safeParseControlMessage;
exports.buildWhisperInit = buildWhisperInit;
exports.tryExtractTranscript = tryExtractTranscript;
...
function buildWhisperInit(params) {
    ...
    return {
        uid,
        model: config_1.FORCED_MODEL,
        language,
        task: translate ? "translate" : "transcribe",
        use_vad,
        initial_prompt: "Transcribe conversational Tagalog and English (Taglish). Keep Filipino words as-is. Preserve names, numbers, and common call-center terms.",
    };
}
function tryExtractTranscript(raw) {
    ...
    const collectTextByKeys = (node, out, depth = 0) => {
        if (node == null)
            return;
        if (depth > 8)
            return;
        ...
        if (Array.isArray(node)) {
            for (const item of node)
                collectTextByKeys(item, out, depth + 1);
            return;
        }
        ...
        const containers = [];
        if (obj.segment)
            containers.push(["segment", obj.segment]);
        if (obj.result)
            containers.push(["result", obj.result]);
        if (obj.data)
            containers.push(["data", obj.data]);
        ...
        for (const [, c] of containers)
            collectTextByKeys(c, out, depth + 1);
    };
    try {
        const parsed = JSON.parse(raw);
        if (typeof parsed !== "object" || parsed === null)
